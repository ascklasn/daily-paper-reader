...
tldr: 病理图像分类中，编码器将图像映射到潜空间，但对标签无关变换的鲁棒性未知。本研究使用通用（Meta）和病理专用（Lunit、Bioptimus）编码器，比较原始与变换后图像嵌入，并与随机嵌入对比。发现变换后嵌入与原嵌入显著接近但非完全不变，表明编码器未完全消除变换影响，解释了数据增强的有效性。通用与病理专用编码器在鲁棒性上存在显著差异，为模型选择和数据增强策略提供依据。
source: arxiv
...
motivation: 探究不同编码器对经典图像变换的鲁棒性，以及变换是否被完全中性化。
method: "使用通用和病理专用编码器，对H&E染色切片施加旋转、翻转等变换，比较嵌入距离。"
result: 变换后嵌入与原嵌入距离显著小于随机嵌入，但非零；病理专用编码器鲁棒性更强。
conclusion: 编码器未完全消除变换效应，数据增强可提升性能；不同编码器鲁棒性差异明显。
---

## 摘要
用于组织病理学分类任务的神经网络训练通常依赖于将数据编码到潜空间，这降低了复杂度并提升了性能。存在多种编码器网络，要么在通用图像数据集（如ImageNET）上预训练，要么专门在组织病理学图像上预训练。编码器网络的训练应适应下游任务，允许编码生物/诊断内容，同时使网络对标签无关的变换保持不变性。本文研究了经典图像变换对潜空间的影响，使用了由Lunit Inc.和Bioptimus（均专注于病理图像）以及Meta研究团队提供的网络。我们通过比较原始和变换后的图像嵌入，并将其与随机的、无关的嵌入进行对比，评估了标准数据变换导致的嵌入方差，使用了结直肠组织数据集和公开可访问的TCGA数据集中的苏木精-伊红染色切片图像块。我们的研究结果表明，原始和变换后图像的嵌入彼此之间比与随机嵌入更接近，表明对变换具有鲁棒性。然而，它们并非完全不变，揭示了编码器网络并未完全中和潜空间中的变换效应，这也解释了为何基于变换的数据增强可以提升性能。在通用编码器网络和组织病理学专用编码器网络之间观察到了显著差异。
