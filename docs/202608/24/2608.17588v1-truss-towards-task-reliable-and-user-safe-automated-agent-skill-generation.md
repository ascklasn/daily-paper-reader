...
tldr: Agent技能将自然语言流程与可执行资源打包，但仅从最终结果评估无法保证其行为安全。TRUSS提出证据引导的技能生成框架，先对照源证据与领域证据检查功能声明，并在九类安全属性下评估完整产物。通过先静态检查再加载智能体运行，TRUSS显著提升技能的功能有效性与安全性，为可复用智能体能力构建提供保障。
source: arxiv
