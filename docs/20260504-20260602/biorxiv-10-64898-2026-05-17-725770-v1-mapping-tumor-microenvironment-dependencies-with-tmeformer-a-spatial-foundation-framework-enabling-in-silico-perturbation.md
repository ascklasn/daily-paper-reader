...
## 速览
**TLDR**：肿瘤微环境的空间背景对肿瘤进展至关重要，但现有虚拟扰动模型忽视空间信息。TMEformer基于高分辨率空间转录组数据，联合建模肿瘤内在程序与微环境信号。在多个肿瘤空间数据中，它不仅优于大规模预训练基线，还能捕获谱系可塑性和治疗耐药等关键转变。该框架可系统扰动识别肿瘤驱动因子，并改善病理空间分层，为空间耦合肿瘤建模提供通用范式。 \
**Motivation**：现有肿瘤虚拟扰动模型忽视空间背景，无法准确捕获肿瘤-微环境依赖关系。 \
**Method**：TMEformer利用高分辨率空间转录组数据，显式建模空间架构，联合学习肿瘤细胞程序与局部微环境信号。 \
**Result**：在多个肿瘤空间数据集上，TMEformer优于大规模预训练模型，准确捕获谱系可塑性和治疗耐药等关键肿瘤转变。 \
**Conclusion**：TMEformer为空间耦合肿瘤生态系统建模和虚拟扰动提供了通用框架，有助于发现新型驱动因子和病理分层。
